...
{
    public class PgCurrentStatusRepository : MainPgRepository, ICurrentStatusRepository
    {
        public PgCurrentStatusRepository(DbSettings _dbSettings, IEventBus _bus) : base(_dbSettings, _bus) { }
        ...
        public ItemResult GetAll()
        {
            ...
            using (NpgsqlConnection connection = this.CreateConnection())
            {
                ...
                {
                    connection.Open();
                    this.CreateFunctionCallQuery(LibraryFunctions.fn_resource_current_statuses_get_all, connection);
                    ...
                    using (dataReader)
                    {
                        ...
                        {
                            itemResult.Item = Newtonsoft.Json.JsonConvert.DeserializeObject<List<CurrentStatus>>((string)dataReader[0]);
                        }
                    }
                    connection.Close();
                }

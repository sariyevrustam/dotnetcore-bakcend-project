    ...
    public class PgResourceRepository : MainPgRepository, IResourceRepository
    {
        ...
        public ItemResult ResourceAuthorSearch(InResourceAuthorSearchFilter inResourceAuthorSearchFilter)
        {
            ...
            using (NpgsqlConnection connection = this.CreateConnection())
            {
                ...
                {
                    connection.Open();
                    this.CreateFunctionCallQuery(LibraryFunctions.fn_resource_author_search, connection);
                    this.Cmd.Parameters.AddWithValue("p_filter", JsonConvert.SerializeObject(inResourceAuthorSearchFilter));
                    NpgsqlDataReader dataReader = null;
                    dataReader = this.Cmd.ExecuteReader();
                    using (dataReader)
                    {
                        while (dataReader.Read())
                        {
                            itemResult.Item = JsonConvert.DeserializeObject<SearchedResourceAuthorResult>((dataReader[0]).ToString());
                        }
                    }
                    connection.Close();
                }
        ...
        public ItemResult ReturnResources(InReturningBookshelfResourceCollection inReturningBookshelfResourceCollection)
        {
            ...
            using (NpgsqlConnection connection = this.CreateConnection())
            {
                ...
                {
                    connection.Open();
                    this.CreateFunctionCallQuery(LibraryFunctions.fn_resource_return, connection);
                    ...
                    using (dataReader)
                    {
                        while (dataReader.Read())
                        {
                            itemResult.Item = (dataReader[0]).ToString();
                        }
                    }
                    connection.Close();
                }

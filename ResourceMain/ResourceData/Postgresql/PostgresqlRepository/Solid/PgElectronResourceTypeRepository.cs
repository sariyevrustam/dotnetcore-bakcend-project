...
{
    public class PgElectronResourceTypeRepository : MainPgRepository, IElectronResourceTypeRepository
    {
        public PgElectronResourceTypeRepository(DbSettings _dbSettings, IEventBus _bus) : base(_dbSettings, _bus) { }
        ...
        public ItemResult GetAll()
        {
            ...
            using (NpgsqlConnection connection = this.CreateConnection())
            {
                ...
                {
                    connection.Open();
                    this.CreateFunctionCallQuery(LibraryFunctions.fn_electron_resource_type_get_all, connection);
                    ...
                    using (dataReader)
                    {
                        ...
                        {
                            itemResult.Item = Newtonsoft.Json.JsonConvert.DeserializeObject<List<ElectronResourceType>>((string)dataReader[0]);
                        }
                    }
                    connection.Close();
                }
